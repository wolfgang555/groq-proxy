...
const CONFIG = {
  ...
  grokApiEndpoint: "https://api.x.ai", // Grok API 端点
};
...
async function handleRequest(request: Request): Promise<Response> {
  ...
  const targetUrl = `${CONFIG.grokApiEndpoint}${pathname}${url.search}`;
...
console.log(`Forwarding requests to: ${CONFIG.grokApiEndpoint}`);
Deno.serve( {
  port: CONFIG.port,
